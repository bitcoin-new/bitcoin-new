...
public:
    CMainParams() {
        ...
        consensus.nBTNSubsidyHalvingInterval = 420000; // 8 years
        consensus.BIP34Height = 227931;
        consensus.BIP34Hash = uint256S("0x000000000000024b89b42a942fe0d9fea3bb44ab7bd1b19115dd6a759c0808b8");
        consensus.BIP65Height = 388381; // 000000000000000004c2b624ed5d7756c508d90fd0da2c7c679febfa6c4735f0
        consensus.BIP66Height = 363725; // 00000000000000000379eaa19dce8c9b722d46ae6a57c2f1a988119488b50931
        consensus.BTNHeight = 501000; // Around 12/25/2017 @ 3:23pm (UTC)
        consensus.BTNPremineWindow = 100;
        consensus.BTNPremineSubsidy = 5250000;
        consensus.BTNSubsidy = 1872;
        consensus.BTNPremineEnforceWhitelist = true;
        ...
        //based on https://github.com/bitcoin-new/bitcoin-new/issues/78
        ...
        nDefaultPort = 8838; // different port than Bitcoin
        ...
        vSeeds.emplace_back("dnsseed.bitcoin-new.org", true);
...
public:
    CTestNetParams() {
        ...
        vSeeds.emplace_back("dnsseed-testnet.bitcoin-new.org", true);
        ...
        vPreminePubkeys = {
                { "mpbpvNW61TVu8bnTFVgN15VHYLNw3K3HzM"},
        };
    }
};
...
public:
    CRegTestParams() {
        ...
        consensus.nBTNSubsidyHalvingInterval = 300;
        consensus.BIP34Height = 100000000; // BIP34 has not activated on regtest (far in the future so block v1 are not rejected in tests)
        consensus.BIP34Hash = uint256();
        consensus.BIP65Height = 1351; // BIP65 activated on regtest (Used in rpc activation tests)
        consensus.BIP66Height = 1251; // BIP66 activated on regtest (Used in rpc activation tests)
        consensus.BTNHeight = 3000;
        consensus.BTNPremineWindow = 100;
        consensus.BTNPremineSubsidy = 5250000;
        consensus.BTNSubsidy = 1872;
        consensus.BTNPremineEnforceWhitelist = false;
        consensus.BitcoinPostforkBlock = uint256();
        consensus.BitcoinPostforkTime = 0;
...
static CScript CltvScript(const std::vector<std::string>& pubkeys) {
    assert(pubkeys.size() == 1);
    CBitcoinAddress address(pubkeys[0]);
...
bool CChainParams::IsPremineAddressScript(const CScript& scriptPubKey, uint32_t height) const {
    assert((uint32_t)consensus.BTNHeight <= height &&
           height < (uint32_t)(consensus.BTNHeight + consensus.BTNPremineWindow));
    if (!consensus.BTNPremineEnforceWhitelist) {
        return true;
    }
    const std::vector<std::string> pubkeys = vPreminePubkeys[0];  // Round robin.
    CScript redeem_script;
        redeem_script = CltvScript(pubkeys);
