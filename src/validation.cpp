...
const std::string strMessageMagic = "Bitcoin New Signed Message:\n";
...
bool static IsBTNHardForkEnabled(int nHeight, const Consensus::Params& params) {
    return nHeight >= params.BTNHeight;
}

bool IsBTNHardForkEnabled(const CBlockIndex* pindexPrev, const Consensus::Params& params) {
    if (pindexPrev == nullptr) {
        return false;
    }

    return IsBTNHardForkEnabled(pindexPrev->nHeight, params);
}

bool IsBTNHardForkEnabledForCurrentBlock(const Consensus::Params& params) {
    AssertLockHeld(cs_main);
    return IsBTNHardForkEnabled(chainActive.Tip(), params);
}
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos, const Consensus::Params& consensusParams)
{
    ...
    bool postfork = block.nHeight >= (uint32_t)consensusParams.BTNHeight;
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    if(nHeight < (uint32_t)consensusParams.BTNHeight){
        ...
        nSubsidy = consensusParams.BTNPremineSubsidy * COIN;

    }else{
        oldHalvings=(uint32_t)(consensusParams.BTNHeight-1) / consensusParams.nSubsidyHalvingInterval;
        halvings = (((nHeight-(uint32_t)(consensusParams.BTNHeight + consensusParams.BTNPremineWindow)) / consensusParams.nBTNSubsidyHalvingInterval));
        // Force block reward to zero when right shift is undefined.
        if (oldHalvings+halvings >= 64)
            return 0;
        nSubsidy = consensusParams.BTNSubsidy * COIN;
        // Subsidy is cut in half every 420,000 blocks which will occur approximately every 8 years.
        nSubsidy >>= halvings;
        int total_halvings=oldHalvings+halvings;
    }
...
bool IsInitialBlockDownload()
{
    ...
    if (chainActive.Tip()->nChainWork < UintToArith256(chainParams.GetConsensus().nMinimumChainWork))
        return true;
    if (fSkipHardforkIBD && chainActive.Tip()->nHeight + 1 >= (int)chainParams.GetConsensus().BTNHeight)
        return false;
    int64_t target_time = fBTNBootstrapping ? (int64_t)chainParams.GetConsensus().BitcoinPostforkTime : GetTime();
...
static unsigned int GetBlockScriptFlags(const CBlockIndex* pindex, const Consensus::Params& consensusparams) {
    ...
    if (IsBTNHardForkEnabled(pindex->pprev, consensusparams)) {
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (block.nHeight < (uint32_t)consensusParams.BTNHeight) {
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Check block height for blocks after BTN fork.
    if (nHeight >= consensusParams.BTNHeight && block.nHeight != (uint32_t)nHeight)
...
static bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    if (nHeight >= consensusParams.BTNHeight &&
        nHeight < consensusParams.BTNHeight + consensusParams.BTNPremineWindow)
    {
