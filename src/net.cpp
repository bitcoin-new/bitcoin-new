    ...
    for (const CDNSSeedData &seed : vSeeds) {
        ...
        if (HaveNameProxy()) {
            ...
            if (LookupHost(host.c_str(), vIPs, 0, true))
            {
                ...
                {
                    int nOneDay = 24*3600;
                    CAddress addr = CAddress(CService(ip, Params().GetDefaultPort(fBTNBootstrapping)), requiredServiceBits);
    ...
    while (!interruptNet)
    {
        ...
        while (!interruptNet)
        {
            ...
            if ((addr.GetPort() != Params().GetDefaultPort() ||
                 (fBTNBootstrapping && addr.GetPort() != Params().GetDefaultPort(true))) && nTries < 50)
                continue;
