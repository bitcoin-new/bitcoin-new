...
bool CanDirectFetch(const Consensus::Params &consensusParams)
{
    ...
    if (fBTNBootstrapping && consensusParams.BitcoinPostforkTime > 0) {
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman& connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    else if (strCommand == NetMsgType::VERSION)
    {
        ...
        if (nVersion < MIN_PEER_PROTO_VERSION)
        {
            ...
            // TODO(h4x3rotab): Disconnect if we already have any BTN block.
        ...
        LogPrintf("receive version message: %s: version %d, blocks=%d, us=%s, peer=%d%s\n",
                  ...
                  remoteAddr);
        if (pfrom->fUsesNewMagic) {
            LogPrintf("peer %d uses New magic in its headers\n", pfrom->id);
        }
    ...
    else if (strCommand == NetMsgType::HEADERS && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        CValidationState state;
        // When bootstrapping BTN network, continue even if there are invalid blocks.
        if (!ProcessNewBlockHeaders(headers, state, chainparams, &pindexLast)) {
            if (fBTNBootstrapping && pindexLast != nullptr) {
...
bool ProcessMessages(CNode* pfrom, CConnman& connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (pfrom->nVersion == 0) {
        if (memcmp(msg.hdr.pchMessageStart, chainparams.MessageStart(),
            ...
            pfrom->fUsesNewMagic = true;
        } else if (fBTNBootstrapping) {
            // Allow to connect to Bitcoin clients when bootstrapping.
            pfrom->fUsesNewMagic = false;
        }
    }
