...
UniValue generateBlocks(std::shared_ptr<CReserveScript> coinbaseScript, int nGenerate, uint64_t nMaxTries, bool keepScript)
{
    ...
    while (nHeight < nHeightEnd)
    {
        ...
        if (pblock->nHeight < (uint32_t)params.GetConsensus().BTNHeight) {
            // Solve sha256d.
...
// NOTE: Unlike wallet RPC (which use BTN values), mining RPCs follow GBT (BIP 22) in using satoshi amounts
...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
        ...
        fLastTemplateSupportsSegwit != fSupportsSegwit)
    {
        ...
        // Create new block
        CScript scriptDummy;
        if (chainActive.Tip()->nHeight+1 >= Params().GetConsensus().BTNHeight &&
                chainActive.Tip()->nHeight+1 < Params().GetConsensus().BTNHeight + Params().GetConsensus().BTNPremineWindow)
        {
            const std::vector<std::string> pubkeys = Params().vPreminePubkeys[0];  // Round robin.
            CBitcoinAddress address(pubkeys[0]);
            scriptDummy = GetScriptForDestination(address.Get());
        }else{
...
UniValue estimatesmartfee(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"feerate\" : x.x,     (numeric, optional) estimate fee-per-kilobyte (in BTN)\n"
